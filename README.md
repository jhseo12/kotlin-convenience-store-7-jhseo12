...
### Model
- [x] 재고 목록 불러오는 기능
- [ ] 구매할 상품 및 수량에 대한 오류 검사 (잘못된 입력, 존재하지 않는 상품, 재고 불충분)
- [ ] 프로모션 할인 기능
  - [ ] 해당되는 상품이 있을 때, 해당 수량보다 고객이 적게 가져왔는지 확인
    - 적게 가져왔을 때
      - [ ] 추가 여부가 Y일 경우, 해당 수량만큼 상품 추가
      - [ ] 추가 여부가 N일 경우, 상품을 추가하지 않는다.
    - 프로모션 재고가 부족할 때
      - [ ] 정가 결제 여부가 Y일 경우, 해당 수량만큼 정가 결제
      - [ ] 정가 결제 여부가 N일 경우, 해당 수량만큼 상품 제외
- [ ] 멤버십 할인 기능
...
- [ ] 뷰로부터 구매할 상품명과 수량을 받아와 모델에게 전처리 요청(예외 처리 검사 등)
